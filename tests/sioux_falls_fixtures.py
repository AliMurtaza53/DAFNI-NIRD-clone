...
from nird.geo_runtime import CONUS_TARGET_CRS, is_conus_albers_alias
...
ANCHOR_LON_LAT = (-96.7313, 43.5460)
NETWORK_EXTENT_M = 15_000.0
...
def build_node_geometries(nodes: pd.DataFrame) -> dict[str, Point]:
    """Treat TNTP X/Y as a local planar shape and anchor it near Sioux Falls."""
    ...
    anchor = (
        gpd.GeoSeries([Point(*ANCHOR_LON_LAT)], crs="EPSG:4326")
        .to_crs(TARGET_CRS)
        .iloc[0]
    )
    scale = NETWORK_EXTENT_M / span
    geometries = {}
    for row in nodes.itertuples(index=False):
        x = anchor.x + (float(row.x) - center[0]) * scale
        y = anchor.y + (float(row.y) - center[1]) * scale
        geometries[str(row.node)] = Point(x, y)
    return geometries
...
def write_hazard_raster(
    ...
    *,
    ...
    for geom in flooded.geometry:
        ...
        # 10 m grid with a half-cell buffer keeps flooding on the bridge only.
...
def build_sioux_falls_dataset(tmp_path: Path) -> tuple[Path, SiouxFallsSpec, gpd.GeoDataFrame]:
    ...
    copy_parameters(toy_data_dir, flow_cap_plph=1)
...
def sioux_falls_env(tmp_path: Path, config_path: Path) -> dict[str, str]:
    return pipeline_env(tmp_path, config_path, network_name="sioux_falls")
