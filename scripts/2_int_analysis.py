...
--------------------
This script is retained for backward compatibility with older UK-oriented workflows.

For current FAF/US workflows, use:
    scripts/2_intersection_analysis.py

Reasons to prefer the newer script include FAF-compatible road-class logic,
US CRS defaults (EPSG:2163), improved raster memory handling, and stronger
CRS/PROJ robustness checks.
"""
...
def main(depth_key, event_key):
    ...
    """
    logging.warning(
        "scripts/2_int_analysis.py is a legacy script. "
        "Prefer scripts/2_intersection_analysis.py for current FAF/US workflows."
    )
