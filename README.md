...
4. **Prepare network (one-time):**
   ...
   This converts FAF5 network format to NIRD-compatible link and node tables.
    ...
    Generate the OD matrix used by Script 1:
    ```bash
    python convert_faf5_od_to_nird.py
    ```
    This writes `faf5_od_matrix.pq` to the census dataset folder (used as Script 1 input).
...
5. **Run the analysis pipeline:**
   ```bash
    # Script 1 reads the OD matrix generated by scripts/run_va_workflow.py.
    # Regenerate that OD first if network/centroid inputs changed.
   ...
   The two `5_xx` sensitivity scripts are intentionally skipped for now and are not included in profiling.
    ...
    Notes:
    - Script 1 reads OD from `data/.../inputs/census_datasets/faf5_od_matrix.pq` (resolved through `config.json`).
    - `scripts/run_va_workflow.py --total-trips 3000000` regenerates the VA-scale synthetic inverse-distance OD matrix used for the Geo-AI final project submission.
