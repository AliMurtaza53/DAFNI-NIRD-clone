...
| 2026-06-08/09 | Patch 5: fused event-candidate writing | Frozen as current baseline | Fused mode writes event candidates inside streaming pass 2 and skips full `temp_flow_matrix` when `NIRD_CREATE_FULL_TEMP_FLOW_MATRIX=0`. The 20k Patch 4 reference vs fused comparison passed exactly for candidate rows, paths, flood links, scalar costs, edge flow state, and pre-event costs. Full-flow one-iteration synthetic smoke completed: 9,678,285 OD rows, 221,214 edge rows, 68,211 event-candidate rows in 2 parts, no full `odpfc`, no global path index, no `UNNEST(e_id)` fallback. Remaining bottleneck is stable `od_id` assignment. |
